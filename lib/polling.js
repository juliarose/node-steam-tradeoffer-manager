...
TradeOfferManager.prototype.doPoll = function(doFullUpdate) {
	...
	// we prefer a longer full update interval
	var ONE_MINUTE = 60 * 1000;
	var fullUpdateInterval = 5 * ONE_MINUTE;
	
	if (Date.now() - this._lastPollFullUpdate >= fullUpdateInterval || doFullUpdate) {
		this._lastPollFullUpdate = Date.now();
		
		fullUpdate = true;
		offersSince = 1;
		
		this._fullUpdated = true;
	}
