...
const CEconItem = require('./EconItem.js');

function TradeOffer(manager, partner, token, data) {
	const EconItem = manager.constructor.EconItem || CEconItem;
	...
	if (data) {
		...
		this.itemsToGive = data.items_to_give || [];
		this.itemsToReceive = data.items_to_receive || [];
		...
		this.tradeID = data.tradeid ? data.tradeid.toString() : null;
		this.fromRealTimeTrade = data.from_real_time_trade;
		this.confirmationMethod = data.confirmation_method || EConfirmationMethod.None;
		this.escrowEnds = data.escrow_end_date ? new Date(data.escrow_end_date * 1000) : null;
		this.rawJson = JSON.stringify(data, null, "\t");
	
		if (manager._language) {
			this.itemsToGive = manager._mapItemsToDescriptions(null, null, this.itemsToGive);
			this.itemsToReceive = manager._mapItemsToDescriptions(null, null, this.itemsToReceive);
		} else {
			this.itemsToGive = processItems(this.itemsToGive);
			this.itemsToReceive = processItems(this.itemsToReceive);
		}
	} else {
		this.id = null;
		this.message = null;
...
TradeOffer.prototype.duplicate = function() {
	var offer = new this.constructor(this.manager, this.partner, this._token);
